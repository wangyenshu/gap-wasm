...
Obj Setupterm( Obj self, Obj tnam ) {
  int i;
  if (!IS_STRING_REP(tnam))
    return False;
  if (setupterm(CSTR_STRING(tnam), 1, &i) != ERR)
    return True;
  else
    return INTOBJ_INT(i);
}
...
Obj WAddnstr(Obj self, Obj num, Obj str, Obj n) {
  ...
  if (!IS_INTOBJ(n))
    ...
    len = INT_INTOBJ(n);
  /* temporary workaround for libncurses 6.5 (e.g., default in Debian 13)
     which returns "false" for empty string */
  if (len == 0)
    return True;
...
static mmask_t mmaskIntlist(Obj list) {
...
static StructGVarFunc GVarFuncs [] = {
  ...
  /* XXX  { "attrsetdefault", 0, "", Attrsetdefault, "ncurses.c:Attrsetdefault" }, */
    ...
    { "setupterm", 1, "termname", Setupterm, "ncurses.c:Setupterm" },
